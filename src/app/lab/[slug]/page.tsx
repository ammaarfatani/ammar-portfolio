...
export default async function LabDetail({ params }: LabDetailProps) { const experiment = getExperiment((await params).slug); if (!experiment) notFound(); return <main className="lab-detail"><Link href="/#ai" className="case-back"><ArrowLeft size={15} />Back</Link><p>{experiment.category}</p><h1>{experiment.title}</h1><p className="lab-detail-description">{experiment.description}</p>{experiment.interactiveUrl && <a href={experiment.interactiveUrl} className="lab-detail-link">Open interactive experience <ArrowUpRight size={16} /></a>}</main>; }
